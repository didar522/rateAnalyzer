...
public class BUPOf_WeekDateCalc {

	ArrayList<WeekCalcTemplate> weekRateData; 
	ArrayList<ReleaseCalendarTemplate> releaseInfo; 
	
	public BUPOf_WeekDateCalc (ArrayList<WeekCalcTemplate> tmp_weekRateData, ArrayList<ReleaseCalendarTemplate> tmp_releaseInfo){
	...
	public void weekDateCalc (){
		...
		while (addDate(7*(weekNumber+1), weekStart).compareTo(endDate)<=0){
			...
			if (addDate(7*weekNumber+3, weekStart).compareTo(releaseInfo.get(releaseNum).getDateReleasEnd())<0){
